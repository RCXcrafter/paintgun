...
public class FluidRenderer {
	 ...
	 * @param light     Light level from TER
	 */
	public static void renderCuboid(PoseStack matrices, VertexConsumer buffer, FluidCuboid cube, Fluid fluid, int light, int packedOverlay) {
		// fluid type, fetch once for all fluids to save effort
		FluidType type = fluid.getFluidType();
		IClientFluidTypeExtensions clientType = IClientFluidTypeExtensions.of(type);
		TextureAtlasSprite still = getBlockSprite(clientType.getStillTexture());
		TextureAtlasSprite flowing = getBlockSprite(clientType.getFlowingTexture());
		boolean isGas = type.isLighterThanAir();
		light = withBlockLight(light, type.getLightLevel());
