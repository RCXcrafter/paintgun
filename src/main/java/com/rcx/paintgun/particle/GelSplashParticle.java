...
@OnlyIn(Dist.CLIENT)
public class GelSplashParticle extends TextureSheetParticle {
	...
	public GelSplashParticle(ClientLevel level, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed, SpriteSet sprites, GelSplashParticleOptions options) {
	...
	public void render(VertexConsumer pBuffer, Camera camera, float pPartialTicks) {
		...
		quaternionf.lookAlong(direction.x, direction.y, direction.z, f, f1, f2).conjugate();
	...
	@OnlyIn(Dist.CLIENT)
	public static class Provider implements ParticleProvider<GelSplashParticleOptions> {
		private final SpriteSet sprites;
		...
		@Override
		public TextureSheetParticle createParticle(GelSplashParticleOptions type, ClientLevel level, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
			return new GelSplashParticle(level, x, y, z, xSpeed, ySpeed, zSpeed, sprites, type);
		}
	}
}
